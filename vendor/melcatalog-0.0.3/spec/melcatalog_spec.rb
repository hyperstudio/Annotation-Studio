...
describe Melcatalog do
  ...
  describe '#search' do
     ...
     it 'search by tag' do
       ...
       fail "No suitable tag available" if tag.empty?
       no_terms = ""
     ...
     it 'limit result count' do
       ...
       fail "Excessive result count" if results[:text].nil? == false && results[:text].size > max
       fail "Excessive result count" if results[:person].nil? == false && results[:person].size > max
       fail "Excessive result count" if results[:artwork].nil? == false && results[:artwork].size > max
     end
