...
function updateTagSelector(tags) {
	var items = "";
	var selected = annotation_studio.selectedTags;
	var tagArray = [];
	for (var key in tags) {
	...
	if (tagArray.length === 0)
		items = "<li>There are no tags in this document to filter by.</li>";
	else {
		tagArray = tagArray.sort();
		for (var i = 0; i < tagArray.length; i++) {
			key = tagArray[i];
			var checked = (selected.indexOf(key) >= 0) ? "checked" : "";
			items += '<li><input type="checkbox" ' + checked + ' id="tag_check_' + key.replace(/\W/g, '') + '" value="' + key + '" /><label for="tag_check_' + key.replace(/\W/g, '') + '">' + key + "</label></li>";
		}
		items = "<li>Select tag(s) to filter on</li>" + items;
	}
	$("#annotation-tag-list").html(items);
}
  ...
  listAnnotations: function(annotationArray) {
    Sidebar.annotations = new Sidebar.LocalAnnotationList(annotationArray);
	  for (var i = 0; i < annotationArray.length; i++) {
		  var tags = annotationArray[i].tags;
		  if (tags) {
			  for (var j = 0; j < tags.length; j++) {
				  this.tagList[tags[j]] = true;
			  }
		  }
	  }
	  updateTagSelector(this.tagList);
