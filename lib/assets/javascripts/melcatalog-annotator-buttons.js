...
tinymce.PluginManager.add('melcatalog', function( editor ) {
    ...
    function search( mode, term ) {
        ...
        var url = "catalog?term=" + encodeURI( term ) + "&types=people,artwork&onlyimages=" + ( ( mode == 'reference' ) ? "false" : "true" );
    ...
    function insertReference( eid, title ) {
        ...
        editor.insertContent(editor.dom.createHTML('a', { href: url }, editor.dom.encode( title ) ));
    }

    function insertImage( eid ) {
        var url = "/documents/catalog/image/" + eid;
        editor.insertContent(editor.dom.createHTML('a', { href: url }, editor.dom.encode( "an image here" ) ));
    }
