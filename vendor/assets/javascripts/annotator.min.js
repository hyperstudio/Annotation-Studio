...
**
** Built at: 2012-06-22 12:25:20Z
*/
((function() {
	var a, b, c, d, e, f, g, h, i, j, k, l, m, n, o, p, q, r, s, t = Array.prototype.slice,
		u = Object.prototype.hasOwnProperty,
		v = function(a, b) {
			function d() {
				this.constructor = a
			}
			for (var c in b) u.call(b, c) && (a[c] = b[c]);
	...
	i = null, typeof Gettext != "undefined" && Gettext !== null ? (l = new Gettext({
		domain: "annotator"
	}), i = function(a) {
		return l.gettext(a)
	}) : i = function(a) {
	...
	}, a.fn.reverse = [].reverse, g = ["log", "debug", "info", "warn", "exception", "assert", "dir", "dirxml", "trace", "group", "groupEnd", "groupCollapsed", "time", "timeEnd", "profile", "profileEnd", "count", "clear", "table", "error", "notifyFirebug", "firebug", "userObjects"];
	if (typeof console != "undefined" && console !== null) {
		...
		for (m = 0, o = g.length; m < o; m++) f = g[m], console[f] == null && (console[f] = function() {
			return console.log(s("Not implemented:") + (" console." + name))
		})
	} else {
		this.console = {};
		for (n = 0, p = g.length; n < p; n++) f = g[n], this.console[f] = function() {};
		this.console.error = function() {
			var a;
			return a = 1 <= arguments.length ? t.call(arguments, 0) : [], alert("ERROR: " + a.join(", "))
		}, this.console.warn = function() {
			var a;
			return a = 1 <= arguments.length ? t.call(arguments, 0) : [], alert("WARNING: " + a.join(", "))
		}
	}
	c = function() {
		function b(b, c) {
		...
		return b.prototype.events = {}, b.prototype.options = {}, b.prototype.element = null, b.prototype.addEvents = function() {
			var a, b, c, d, e, f, g, h;
			f = this.events, h = [];
			for (c in f) b = f[c], g = c.split(" "), d = 2 <= g.length ? t.call(g, 0, e = g.length - 1) : (e = 0, []), a = g[e++], h.push(this.addEvent(d.join(" "), a, b));
			return h
		...
		return b = function() {
			...
			for (a in b) {
				if (!u.call(b, a)) continue;
				c = b[a], d.push(a)
			}
			return d
		}(), "blur focus focusin focusout load resize scroll unload click dblclick\nmousedown mouseup mousemove mouseover mouseout mouseenter mouseleave\nchange select submit keydown keypress keyup error".split(/[^a-z]+/).concat(b)
	}(), e = {}, e.sniff = function(a) {
		return a.commonAncestorContainer != null ? new e.BrowserRange(a) : typeof a.start == "string" ? new e.SerializedRange(a) : a.start && typeof a.start == "object" ? new e.NormalizedRange(a) : (console.error(s("Could not sniff range type")), !1)
		...
		return a.prototype.normalize = function(a) {
			...
			if (this.tainted) return console.error(s("You may only call normalize() once on a BrowserRange!")), !1;
		...
		return b.prototype.normalize = function(b) {
			...
			return c = document.compareDocumentPosition == null ?
			function(a, b) {
				return a.contains(b)
			} : function(a, b) {
				return a.compareDocumentPosition(b) & 16
			}, a(h.startContainer).parents().reverse().each(function() {
		...
		return v(c, b), c.prototype.events = {
			".annotator-adder button click": "onAdderClick",
			".annotator-adder button mousedown": "onAdderMousedown",
			".annotator-hl mouseover": "onHighlightMouseover",
			".annotator-hl mouseout": "startViewerHideTimer"
		}, c.prototype.html = {
			adder: '<div class="annotator-adder"><button>' + s("Annotate") + "</button></div>",
			...
			return this.viewer = new c.Viewer({
				...
				load: function(c, d) {
					return d.text ? a(c).escape(d.text) : a(c).html("<i>" + s("No Comment") + "</i>"), b.publish("annotationViewerTextField", [c, d])
				}
			...
			var b, c, d, e;
			return d = a("#annotator-dynamic-style"), d.length || (d = a('<style id="annotator-dynamic-style"></style>').appendTo(document.head)), c = "*" +
			function() {
				var a, b, c, d;
				c = ["adder", "outer", "notice", "filter"], d = [];
				for (a = 0, b = c.length; a < b; a++) e = c[a], d.push(":not(.annotator-" + e + ")");
				return d
			}().join(""), b = j.maxZIndex(a(document.body).find(c)), b = Math.max(b, 1e3), d.text([".annotator-adder, .annotator-outer, .annotator-notice {", "  z-index: " + (b + 20) + ";", "}", ".annotator-filter {", "  z-index: " + (b + 10) + ";", "}"].join("\n")), this
		}, c.prototype.getSelectedRanges = function() {
			var b, c, d, f, g, h, i, k, l;
			i = j.getGlobal().getSelection(), g = [], h = [], i.isCollapsed || (g = function() {
				var a, g;
				g = [];
			...
			for (k = 0, l = h.length; k < l; k++) f = h[k], i.addRange(f);
			...
			return this.plugins.Store ? this.plugins.Store.dumpAnnotations() : console.warn(s("Can't dump annotations without Store plugin."))
			...
			return b && this.selectedRanges.length ? this.adder.css(j.mousePosition(b, this.wrapper[0])).show() : this.adder.hide()
			...
			}), this.showViewer(a.makeArray(c), j.mousePosition(b, this.wrapper[0])))
		...
		return v(b, a), b.prototype.pluginInit = function() {}, b
	}(c), h = j.getGlobal(), ((r = h.document) != null ? r.evaluate : void 0) == null && a.getScript("http://assets.annotateit.org/vendor/xpath.min.js"), h.getSelection == null && a.getScript("http://assets.annotateit.org/vendor/ierange.min.js"), h.JSON == null && a.getScript("http://assets.annotateit.org/vendor/json2.min.js"), b.$ = a, b.Delegator = c, b.Range = e, b._t = s, b.supported = function() {
		return function() {
			return !!this.getSelection
		}()
	}, b.noConflict = function() {
		return j.getGlobal().Annotator = k, this
		...
		return v(d, c), d.prototype.classes = {
			hide: "annotator-hide",
			invert: {
			...
			var b, c, d, e, f;
			return this.resetOrientation(), f = a(j.getGlobal()), e = this.element.children(":first"), c = e.offset(), d = {
				top: f.scrollTop(),
		...
		return v(c, b), c.prototype.events = {
			...
			return j.preventEventDefault(a), this.element.removeClass(this.classes.hide), this.element.find(".annotator-save").addClass(this.classes.focus), this.checkOrientation(), this.element.find(":input:first").focus(), this.setupDraggables(), this.publish("show")
		}, c.prototype.hide = function(a) {
			return j.preventEventDefault(a), this.element.addClass(this.classes.hide), this.publish("hide")
			...
			var b, c, d, e;
			j.preventEventDefault(a), e = this.fields;
			...
			case "input":
			case "radio":
				e = a("<input name='permission-chooser' />")
			}
			return c.append(e), e.attr({
				id: d.id,
				placeholder: d.label
			}), d.type === "radio" && (e[0].type = "radio", c.addClass("annotator-radio"), c.append(a("<label />", {
				"for": d.id,
				html: d.label
		...
		function c(b) {
			this.onDeleteClick = w(this.onDeleteClick, this), this.onEditClick = w(this.onEditClick, this), this.load = w(this.load, this), this.hide = w(this.hide, this), this.show = w(this.show, this), c.__super__.constructor.call(this, a(this.html.element)[0], b), this.item = a(this.html.item)[0], this.fields = [], this.annotations = []
		}
		return v(c, b), c.prototype.events = {
			...
			var b, c = this;
			return j.preventEventDefault(a), b = this.element.find(".annotator-controls").addClass(this.classes.showControls), setTimeout(function() {
		...
		function d(b) {
			this.hide = w(this.hide, this), this.show = w(this.show, this), d.__super__.constructor.call(this, a(this.options.html).appendTo(document.body)[0], b)
		}
		return v(d, c), d.prototype.events = {
			click: "hide"
		}, d.prototype.options = {
